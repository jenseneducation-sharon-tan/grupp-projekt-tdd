import { shallowMount,mount } from '@vue/test-utils'
import AddDeleteButton from '@/components/AddDeleteButton.vue'


/* describe("AddDeleteButton", () => {
    let wrapper,cart
    beforeEach(() => {
        cart= {name:"Banan",  id:1, price: 23, unit: "kg", count: 0}
         wrapper = shallowMount(AddDeleteButton,{propsData:{ cart}})
    });
    it("should add product to cart when clicked", async () => {
      const expectedValue = 1;
      const button = wrapper.find('#add');
      await button.trigger('click');
      let actual = product.count
      expect(actual).toBe(expectedValue);
	}) */
	
	describe("AddDeleteButton", async() => {
		let wrapper, cart;
		beforeEach(() => {
		  cart = [
			{ name: "Banan", id: 1, image: "", price: 23, unit: "kg", count: 0 },
			{ name: "Jordgubbar", id: 2, image: "", price: 99, unit: "kg", count: 0 },
			{ name: "Ananas", id: 3, image: "", price: 15, unit: "st", count: 0 },
			{ name: "Blåbär", id: 4, image: "", price: 45, unit: "kg", count: 0 },
			{ name: "Avokado", id: 5, image: "", price: 10, unit: "st", count: 0 },
			{ name: "Äpple", id: 6, image: "", price: 24.9, unit: "kg", count: 0 },
			{ name: "Vindruvor grön", id: 7, image: "", price: 30, unit: "kg", count: 0 },
			{ name: "Päron", id: 8, image: "", price: 23, unit: "kg", count: 0 },
		  ]
		  wrapper = mount(AddDeleteButton, { propsData: { cart } });
		});
		it("should have to show  1 product when added to cart", async () => {
		  const expected = 1;
		  const button = wrapper.find('#add');
     	 await button.trigger('click');
		  const actual = wrapper.findAll(".cart").length;
		  expect(actual).toBe(expected);
		});
	  });